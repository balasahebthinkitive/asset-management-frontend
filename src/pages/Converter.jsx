...
import { getConverters, createConverter, updateConverter, deleteConverter } from "../api/converters";
import "./Laptops.css";
...
export default function Converter() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const res = await getConverters();
      const items = res.data?.converters ?? res.data ?? [];
  ...
  const save=async()=>{if(!form.type){alert("Type required.");return;}try{setSaving(true);if(editId){await updateConverter(editId,form);}else{await createConverter(form);}await fetchData();setShowModal(false);setEditId(null);}catch(err){setError(err.response?.data?.message||"Save failed.");}finally{setSaving(false);}};

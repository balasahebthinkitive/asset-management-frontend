...
import INITIAL_DATA from "../data/ramData";
...
const EMPTY_FORM = { brand:"", type:"DDR4", size:"32GB", serial:"", assigned:"", status:"Available", location:"" };
const BRANDS    = ["Crucial","Samsung","Value Memory","SK Hynix","Ramaxel","Other"];
const TYPES     = ["DDR4","DDR5"];
const SIZES     = ["8GB","16GB","32GB"];
...
export default function RAM() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const items = res.data?.ram ?? res.data ?? [];
  ...
  const filtered = useMemo(() => {
    return assets.filter(a => {
      ...
      const matchSearch = !q || [a.brand,a.type,a.size,a.serial,a.assigned,a.location].some(v => v?.toLowerCase().includes(q));
      const matchBrand  = !filterBrand  || a.brand === filterBrand;
      const matchType   = !filterType   || a.type === filterType;
      const matchSize   = !filterSize   || a.size === filterSize;
      const matchStatus = !filterStatus || a.status === filterStatus;
      return matchSearch && matchBrand && matchType && matchSize && matchStatus;
    });
  }, [assets, search, filterBrand, filterType, filterSize, filterStatus]);

  const columns = useMemo(() => [
    { accessorKey: "id",       header: "#",            size: 50  },
    { accessorKey: "brand",    header: "Brand",        size: 120 },
    { accessorKey: "type",     header: "Type",         size: 80,
      cell: i => <span className="lp-type-badge" style={{ background: i.getValue()==="DDR5"?"#EEEDFE":"#E6F1FB", color: i.getValue()==="DDR5"?"#3C3489":"#0C447C" }}>{i.getValue()}</span> },
    { accessorKey: "size",     header: "Size",         size: 70  },
    { accessorKey: "serial",   header: "Serial No",    size: 220,
      cell: i => <span className="lp-mono">{i.getValue()}</span> },
    { accessorKey: "assigned", header: "Installed In", size: 180 },
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      if (editId) { await updateRAM(editId, form); } else { await createRAM(form); }
      await fetchData();
      closeModal();
    } catch (err) {
  ...
  const stats = {
    ...
    ddr5:      assets.filter(a => a.type === "DDR5").length,
  };
    ...
    <div className="lp-page">
      ...
      <div className="lp-page-header" style={{ padding:"16px 20px 0" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:10, marginBottom:12 }}>
          <div>
            <h1 style={{ margin:0, fontSize:20, fontWeight:700, color:"#111827", lineHeight:1.2 }}>RAM Modules</h1>
            <p style={{ margin:"2px 0 0", fontSize:12, color:"#6b7280" }}>{assets.length} total modules</p>
          </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10, marginBottom:14 }}>
          {[
            { label:"Total Modules",        val:stats.total,    color:"#2878C8", border:"#2878C8" },
            { label:"Assigned / Installed", val:stats.assigned, color:"#2878C8", border:"#2878C8" },
            { label:"In-Store / Available", val:stats.available,color:"#6a7a00", border:"#b0c21f" },
            { label:"DDR5 Modules",         val:stats.ddr5,     color:"#d97706", border:"#d97706" },
          ].map(s => (
      ...
      <div className="lp-card" style={{ margin:"0 20px 20px" }}>
        ...
        <div style={{ padding:"10px 14px", borderBottom:"1px solid #E5E7EB", display:"flex", gap:8, flexWrap:"wrap", alignItems:"center", background:"#fafafa" }}>
          ...
          <select className="lp-sel lp-sel-sm" value={filterSize}
            onChange={e => { setFilterSize(e.target.value); setPagination(p => ({...p, pageIndex:0})); }}>
          ...
          {(search || filterBrand || filterType || filterSize || filterStatus) && (
            <button className="lp-btn-ghost" onClick={() => { setSearch(""); setFilterBrand(""); setFilterType(""); setFilterSize(""); setFilterStatus(""); setPagination({pageIndex:0,pageSize:15}); }}>
              Clear
            </button>
          )}
          ...
          <table className="lp-table">
            ...
            <tbody>
              {table.getRowModel().rows.length === 0 ? (
                <tr><td colSpan={columns.length} className="lp-empty">No modules found</td></tr>
        ...
        <div className="lp-overlay">
          <div className="lp-modal">
            <div className="lp-modal-header">
              <h2 className="lp-modal-title">{editId ? "Edit Module" : "Add New Module"}</h2>
            ...
            <div className="lp-modal-body">
              <div className="lp-form-grid">
                ...
                <Field field="size"     label="Size"                       opts={SIZES}     />
                <Field field="serial"   label="Serial No *"                                 />
                <Field field="status"   label="Status"                     opts={STATUSES}  />
                <Field field="assigned" label="Installed In / Assigned To"                  />
                <Field field="location" label="Location"                   opts={LOCATIONS} />
              </div>
            </div>
            <div className="lp-modal-footer">
              <button className="lp-btn-ghost" onClick={closeModal}>Cancel</button>
              <button className="lp-btn-primary" onClick={handleSave} disabled={saving}>{saving ? "Saving…" : editId ? "Update Module" : "Save Module"}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {deleteConfirm && (
        <div className="lp-overlay">
          <div className="lp-modal lp-modal-sm">
            <div className="lp-modal-header">
              <h3 className="lp-modal-title">Delete Module</h3>
              <button className="lp-modal-close" onClick={() => setDeleteConfirm(null)}>×</button>
            </div>
            <div className="lp-modal-body">
              <p className="lp-del-msg">
                Are you sure you want to delete <strong>{deleteConfirm.brand} {deleteConfirm.size}</strong> ({deleteConfirm.serial})?<br />
                Installed in: <strong>{deleteConfirm.assigned}</strong>
              </p>
            </div>
            <div className="lp-modal-footer">

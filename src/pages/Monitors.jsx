...
import INITIAL_DATA from "../data/monitorsData";
import { getMonitors, createMonitor, updateMonitor, deleteMonitor } from "../api/monitors";
import "./Laptops.css";

const BADGE={Assigned:{bg:"rgba(40,120,200,0.12)",color:"#2878C8"},Available:{bg:"rgba(176,194,31,0.15)",color:"#6a7a00"},Maintenance:{bg:"#fef3c7",color:"#92400e"},"Not Working":{bg:"#fee2e2",color:"#991b1b"}};
const EMPTY={brand:"",model:"",size:"",serial:"",assigned:"",status:"Available",location:"",remark:""};
const BRANDS=["Dell","LG","Samsung","HP","Lenovo","ViewSonic","BenQ","Acer","Other"];
const SIZES=["19\"","21\"","22\"","24\"","27\"","32\"","Other"];
const STATUSES=["Assigned","Available","Maintenance","Not Working"];
...
export default function Monitors() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const items = res.data?.monitors ?? res.data ?? [];
  ...
  const filtered=useMemo(()=>assets.filter(a=>{const q=search.toLowerCase();return(!q||[a.brand,a.model,a.serial,a.assigned].some(v=>v?.toLowerCase().includes(q)))&&(!fBrand||a.brand===fBrand)&&(!fSize||a.size===fSize)&&(!fStatus||a.status===fStatus);}),[assets,search,fBrand,fSize,fStatus]);
  const columns=useMemo(()=>[
    {accessorKey:"id",header:"#",size:50},
    {accessorKey:"brand",header:"Brand",size:100},
    {accessorKey:"model",header:"Model",size:150},
    {accessorKey:"size",header:"Size",size:70},
    {accessorKey:"serial",header:"Serial No",size:180,cell:i=><span className="lp-mono">{i.getValue()||"—"}</span>},
    {accessorKey:"assigned",header:"Assigned To",size:190},
    {accessorKey:"status",header:"Status",size:120,cell:i=>{const b=BADGE[i.getValue()]||BADGE.Available;return<span className="lp-status-badge" style={b}>{i.getValue()}</span>;}},
  ...
  const save=async()=>{if(!form.brand||!form.model){alert("Brand and Model required.");return;}try{setSaving(true);if(editId){await updateMonitor(editId,form);}else{await createMonitor(form);}await fetchData();setShowModal(false);setEditId(null);}catch(err){setError(err.response?.data?.message||"Save failed.");}finally{setSaving(false);}};

  const stats={total:assets.length,assigned:assets.filter(a=>a.status==="Assigned").length,available:assets.filter(a=>a.status==="Available").length,issues:assets.filter(a=>["Maintenance","Not Working"].includes(a.status)).length};
    ...
    <div className="lp-page">
      ...
      <div className="lp-page-header" style={{padding:"16px 20px 0"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10,marginBottom:12}}>
          <div><h1 style={{margin:0,fontSize:20,fontWeight:700,color:"#111827"}}>Monitors</h1><p style={{margin:"2px 0 0",fontSize:12,color:"#6b7280"}}>{assets.length} total monitors</p></div>
          <button className="lp-add-btn" style={{borderRadius:7,padding:"8px 18px"}} onClick={()=>{setForm(EMPTY);setEditId(null);setShowModal(true);}}>+ Add New Monitor</button>
        </div>
      ...
      <div className="lp-card" style={{margin:"0 20px 20px"}}>
        <div style={{padding:"10px 14px",borderBottom:"1px solid #E5E7EB",display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",background:"#fafafa"}}>
          <input className="lp-search" placeholder="Search brand, model, serial…" value={search} onChange={e=>{setSearch(e.target.value);setPagination(p=>({...p,pageIndex:0}));}} style={{width:220}}/>
          ...
          <select className="lp-sel lp-sel-sm" value={fSize} onChange={e=>{setFSize(e.target.value);setPagination(p=>({...p,pageIndex:0}));}}><option value="">All sizes</option>{SIZES.map(s=><option key={s} value={s}>{s}</option>)}</select>
          ...
          {(search||fBrand||fSize||fStatus)&&<button className="lp-btn-ghost" onClick={()=>{setSearch("");setFBrand("");setFSize("");setFStatus("");setPagination({pageIndex:0,pageSize:15});}}>Clear</button>}
          <span style={{fontSize:12,color:"#9ca3af",marginLeft:"auto"}}>{total} results</span>
        </div>
        <div className="lp-table-wrap"><table className="lp-table"><thead>{table.getHeaderGroups().map(hg=><tr key={hg.id}>{hg.headers.map(h=><th key={h.id} className="lp-th" onClick={h.column.getToggleSortingHandler()} style={{cursor:h.column.getCanSort()?"pointer":"default",width:h.getSize()}}>{flexRender(h.column.columnDef.header,h.getContext())}<span className="lp-sort-hint">{h.column.getIsSorted()==="asc"?" ↑":h.column.getIsSorted()==="desc"?" ↓":h.column.getCanSort()?" ↕":""}</span></th>)}</tr>)}</thead><tbody>{table.getRowModel().rows.length===0?<tr><td colSpan={columns.length} className="lp-empty">No monitors found</td></tr>:table.getRowModel().rows.map((row,i)=><tr key={row.id} className={`lp-tr${i%2!==0?" lp-tr-alt":""}`}>{row.getVisibleCells().map(cell=><td key={cell.id} className="lp-td">{flexRender(cell.column.columnDef.cell,cell.getContext())}</td>)}</tr>)}</tbody></table></div>
      ...
      {showModal&&<div className="lp-overlay"><div className="lp-modal"><div className="lp-modal-header"><h2 className="lp-modal-title">{editId?"Edit Monitor":"Add New Monitor"}</h2><button className="lp-modal-close" onClick={()=>{setShowModal(false);setEditId(null);}}>×</button></div><div className="lp-modal-body"><div className="lp-form-grid"><Field field="brand" label="Brand *" opts={BRANDS}/><Field field="model" label="Model *"/><Field field="size" label="Size" opts={SIZES}/><Field field="serial" label="Serial No"/><Field field="status" label="Status" opts={STATUSES}/><Field field="assigned" label="Assigned To"/><Field field="location" label="Location" opts={LOCATIONS}/><Field field="remark" label="Remark"/></div></div><div className="lp-modal-footer"><button className="lp-btn-ghost" onClick={()=>{setShowModal(false);setEditId(null);}}>Cancel</button><button className="lp-btn-primary" onClick={save} disabled={saving}>{saving?"Saving…":editId?"Update":"Save"}</button></div></div></div>}
      {delConfirm&&<div className="lp-overlay"><div className="lp-modal lp-modal-sm"><div className="lp-modal-header"><h3 className="lp-modal-title">Delete Monitor</h3><button className="lp-modal-close" onClick={()=>setDelConfirm(null)}>×</button></div><div className="lp-modal-body"><p className="lp-del-msg">Delete <strong>{delConfirm.brand} {delConfirm.model}</strong>{delConfirm.size?` ${delConfirm.size}`:""}?</p></div><div className="lp-modal-footer"><button className="lp-btn-ghost" onClick={()=>setDelConfirm(null)}>Cancel</button><button className="lp-btn-danger" onClick={async()=>{try{await deleteMonitor(delConfirm._id||delConfirm.id);setDelConfirm(null);await fetchData();}catch(err){setError(err.response?.data?.message||"Delete failed.");setDelConfirm(null);}}}>Delete</button></div></div></div>}
    </div>
  );
}

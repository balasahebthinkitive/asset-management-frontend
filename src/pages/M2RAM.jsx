...
const EMPTY = { type:"M.2 SSD", brand:"", capacity:"", serial:"", assigned:"", status:"Available", location:"", remark:"" };
const TYPES    = ["M.2 SSD","DDR3","DDR4"];
const BRANDS   = ["Samsung","WD","Crucial","SK Hynix","Kingston","Other"];
const CAPACITIES=["128GB","256GB","512GB","1TB","2TB","4GB","8GB","16GB","32GB","Other"];
...
export default function M2RAM() {
  ...
  const filtered = useMemo(()=>assets.filter(a=>{
    ...
    return (!q||[a.brand,a.type,a.capacity,a.serial,a.assigned].some(v=>v?.toLowerCase().includes(q)))
      &&(!fType||a.type===fType)&&(!fBrand||a.brand===fBrand)&&(!fStatus||a.status===fStatus);
  }),[assets,search,fType,fBrand,fStatus]);

  const columns = useMemo(()=>[
    {accessorKey:"id",header:"#",size:50},
    {accessorKey:"type",header:"Type",size:90,cell:i=><span className="lp-type-badge" style={{background:i.getValue().includes("SSD")?"#ECFDF5":"#EEF2FF",color:i.getValue().includes("SSD")?"#065F46":"#3730A3"}}>{i.getValue()}</span>},
    {accessorKey:"brand",header:"Brand",size:110},
    {accessorKey:"capacity",header:"Capacity",size:90},
    {accessorKey:"serial",header:"Serial No",size:200,cell:i=><span className="lp-mono">{i.getValue()||"—"}</span>},
    {accessorKey:"assigned",header:"Installed In",size:200},
    ...
    <div className="lp-page">
      <div className="lp-page-header" style={{padding:"16px 20px 0"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10,marginBottom:12}}>
          ...
          <button className="lp-add-btn" style={{borderRadius:7,padding:"8px 18px"}} onClick={()=>{setForm(EMPTY);setEditId(null);setShowModal(true);}}>+ Add New Module</button>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:14}}>
          {[{label:"Total",val:stats.total,color:"#2878C8",border:"#2878C8"},{label:"Assigned",val:stats.assigned,color:"#2878C8",border:"#2878C8"},{label:"Available",val:stats.available,color:"#6a7a00",border:"#b0c21f"},{label:"M.2 SSD",val:stats.ssd,color:"#065F46",border:"#065F46"}].map(s=>(
      ...
      <div className="lp-card" style={{margin:"0 20px 20px"}}>
        <div style={{padding:"10px 14px",borderBottom:"1px solid #E5E7EB",display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",background:"#fafafa"}}>
          ...
          {(search||fType||fBrand||fStatus)&&<button className="lp-btn-ghost" onClick={()=>{setSearch("");setFType("");setFBrand("");setFStatus("");setPagination({pageIndex:0,pageSize:15});}}>Clear</button>}
        ...
        <div className="lp-table-wrap"><table className="lp-table">
          ...
          <tbody>{table.getRowModel().rows.length===0?<tr><td colSpan={columns.length} className="lp-empty">No modules found</td></tr>:table.getRowModel().rows.map((row,i)=><tr key={row.id} className={`lp-tr${i%2!==0?" lp-tr-alt":""}`}>{row.getVisibleCells().map(cell=><td key={cell.id} className="lp-td">{flexRender(cell.column.columnDef.cell,cell.getContext())}</td>)}</tr>)}</tbody>
        </table></div>

...
export default function ClientDevices() {
  ...
  const save=()=>{if(!form.client){alert("Client name required.");return;}if(editId)setAssets(p=>p.map(a=>a.id===editId?{...form,id:editId}:a));else{const id=Math.max(...assets.map(a=>a.id),0)+1;setAssets(p=>[...p,{...form,id}]);}setShowModal(false);setEditId(null);};

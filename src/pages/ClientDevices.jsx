...
import INITIAL_DATA from "../data/clientDevicesData";
import { getClientDevices, createClientDevice, updateClientDevice, deleteClientDevice } from "../api/clientDevices";
import "./Laptops.css";

const BADGE={Assigned:{bg:"rgba(40,120,200,0.12)",color:"#2878C8"},Available:{bg:"rgba(176,194,31,0.15)",color:"#6a7a00"},Returned:{bg:"#d1fae5",color:"#065f46"},Maintenance:{bg:"#fef3c7",color:"#92400e"},"Not Working":{bg:"#fee2e2",color:"#991b1b"}};
const EMPTY={type:"Laptop",brand:"",model:"",serial:"",client:"",company:"",startDate:"",endDate:"",status:"Assigned",location:"",remark:""};
const TYPES=["Laptop","Desktop","Mobile","Tablet","Monitor","Other"];
const STATUSES=["Assigned","Available","Returned","Maintenance","Not Working"];
...
export default function ClientDevices() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const res = await getClientDevices();
      const items = res.data?.devices ?? res.data ?? [];
  ...
  const filtered=useMemo(()=>assets.filter(a=>{const q=search.toLowerCase();return(!q||[a.type,a.brand,a.model,a.serial,a.client,a.company].some(v=>v?.toLowerCase().includes(q)))&&(!fType||a.type===fType)&&(!fStatus||a.status===fStatus)&&(!fLoc||a.location===fLoc);}),[assets,search,fType,fStatus,fLoc]);
  const columns=useMemo(()=>[
    ...
    {accessorKey:"client",header:"Client Name",size:160},
    {accessorKey:"company",header:"Company",size:140},
    {accessorKey:"startDate",header:"Start Date",size:100,cell:i=><span className="lp-mono">{i.getValue()||"—"}</span>},
    {accessorKey:"endDate",header:"End Date",size:100,cell:i=><span className="lp-mono">{i.getValue()||"—"}</span>},
    {accessorKey:"status",header:"Status",size:110,cell:i=>{const b=BADGE[i.getValue()]||BADGE.Assigned;return<span className="lp-status-badge" style={b}>{i.getValue()}</span>;}},
  ...
  const save=async()=>{if(!form.client){alert("Client name required.");return;}try{setSaving(true);if(editId){await updateClientDevice(editId,form);}else{await createClientDevice(form);}await fetchData();setShowModal(false);setEditId(null);}catch(err){setError(err.response?.data?.message||"Save failed.");}finally{setSaving(false);}};

  const stats={total:assets.length,assigned:assets.filter(a=>a.status==="Assigned").length,returned:assets.filter(a=>a.status==="Returned").length,issues:assets.filter(a=>["Maintenance","Not Working"].includes(a.status)).length};
    ...
    <div className="lp-page">
      ...
      <div className="lp-page-header" style={{padding:"16px 20px 0"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10,marginBottom:12}}>
          <div><h1 style={{margin:0,fontSize:20,fontWeight:700,color:"#111827"}}>Client Devices</h1><p style={{margin:"2px 0 0",fontSize:12,color:"#6b7280"}}>{assets.length} total devices</p></div>
          <button className="lp-add-btn" style={{borderRadius:7,padding:"8px 18px"}} onClick={()=>{setForm(EMPTY);setEditId(null);setShowModal(true);}}>+ Add New Device</button>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:14}}>
          {[{label:"Total",val:stats.total,color:"#2878C8",border:"#2878C8"},{label:"Assigned",val:stats.assigned,color:"#2878C8",border:"#2878C8"},{label:"Returned",val:stats.returned,color:"#065f46",border:"#10b981"},{label:"Issues",val:stats.issues,color:"#dc2626",border:"#dc2626"}].map(s=>(
      ...
      <div className="lp-card" style={{margin:"0 20px 20px"}}>
        <div style={{padding:"10px 14px",borderBottom:"1px solid #E5E7EB",display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",background:"#fafafa"}}>
          <input className="lp-search" placeholder="Search client, company, serial…" value={search} onChange={e=>{setSearch(e.target.value);setPagination(p=>({...p,pageIndex:0}));}} style={{width:230}}/>
        ...
        <div className="lp-table-wrap"><table className="lp-table"><thead>{table.getHeaderGroups().map(hg=><tr key={hg.id}>{hg.headers.map(h=><th key={h.id} className="lp-th" onClick={h.column.getToggleSortingHandler()} style={{cursor:h.column.getCanSort()?"pointer":"default",width:h.getSize()}}>{flexRender(h.column.columnDef.header,h.getContext())}<span className="lp-sort-hint">{h.column.getIsSorted()==="asc"?" ↑":h.column.getIsSorted()==="desc"?" ↓":h.column.getCanSort()?" ↕":""}</span></th>)}</tr>)}</thead><tbody>{table.getRowModel().rows.length===0?<tr><td colSpan={columns.length} className="lp-empty">No client devices found</td></tr>:table.getRowModel().rows.map((row,i)=><tr key={row.id} className={`lp-tr${i%2!==0?" lp-tr-alt":""}`}>{row.getVisibleCells().map(cell=><td key={cell.id} className="lp-td">{flexRender(cell.column.columnDef.cell,cell.getContext())}</td>)}</tr>)}</tbody></table></div>
      ...
      {showModal&&<div className="lp-overlay"><div className="lp-modal"><div className="lp-modal-header"><h2 className="lp-modal-title">{editId?"Edit Device":"Add New Device"}</h2><button className="lp-modal-close" onClick={()=>{setShowModal(false);setEditId(null);}}>×</button></div><div className="lp-modal-body"><div className="lp-form-grid"><Field field="type" label="Type *" opts={TYPES}/><Field field="brand" label="Brand"/><Field field="model" label="Model"/><Field field="serial" label="Serial No"/><Field field="client" label="Client Name *"/><Field field="company" label="Company"/><Field field="startDate" label="Start Date" type="date"/><Field field="endDate" label="End Date" type="date"/><Field field="status" label="Status" opts={STATUSES}/><Field field="location" label="Location" opts={LOCATIONS}/><Field field="remark" label="Remark"/></div></div><div className="lp-modal-footer"><button className="lp-btn-ghost" onClick={()=>{setShowModal(false);setEditId(null);}}>Cancel</button><button className="lp-btn-primary" onClick={save} disabled={saving}>{saving?"Saving…":editId?"Update":"Save"}</button></div></div></div>}
      {delConfirm&&<div className="lp-overlay"><div className="lp-modal lp-modal-sm"><div className="lp-modal-header"><h3 className="lp-modal-title">Delete Device</h3><button className="lp-modal-close" onClick={()=>setDelConfirm(null)}>×</button></div><div className="lp-modal-body"><p className="lp-del-msg">Delete <strong>{delConfirm.type} — {delConfirm.brand} {delConfirm.model}</strong>{delConfirm.client?<><br/>Client: <strong>{delConfirm.client}</strong></>:null}?</p></div><div className="lp-modal-footer"><button className="lp-btn-ghost" onClick={()=>setDelConfirm(null)}>Cancel</button><button className="lp-btn-danger" onClick={async()=>{try{await deleteClientDevice(delConfirm._id||delConfirm.id);setDelConfirm(null);await fetchData();}catch(err){setError(err.response?.data?.message||"Delete failed.");setDelConfirm(null);}}}>Delete</button></div></div></div>}
    </div>
  );
}

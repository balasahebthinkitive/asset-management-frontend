...
import { getHDDs, createHDD, updateHDD, deleteHDD } from "../api/hddStorage";
import "./Laptops.css";
...
export default function HDDStorage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const res = await getHDDs();
      const items = res.data?.hdds ?? res.data ?? [];
  ...
  const save=async()=>{if(!form.brand){alert("Brand required.");return;}try{setSaving(true);if(editId){await updateHDD(editId,form);}else{await createHDD(form);}await fetchData();setShowModal(false);setEditId(null);}catch(err){setError(err.response?.data?.message||"Save failed.");}finally{setSaving(false);}};

...
export default function ExtraEquipment() {
  ...
  const save=()=>{if(!form.category){alert("Category required.");return;}if(editId)setAssets(p=>p.map(a=>a.id===editId?{...form,id:editId}:a));else{const id=Math.max(...assets.map(a=>a.id),0)+1;setAssets(p=>[...p,{...form,id}]);}setShowModal(false);setEditId(null);};

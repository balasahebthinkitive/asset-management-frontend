...
import INITIAL_DATA from "../data/extraEquipData";
import { getExtraEquipment, createExtraEquipment, updateExtraEquipment, deleteExtraEquipment } from "../api/extraEquipment";
import "./Laptops.css";

const BADGE={Assigned:{bg:"rgba(40,120,200,0.12)",color:"#2878C8"},Available:{bg:"rgba(176,194,31,0.15)",color:"#6a7a00"},Maintenance:{bg:"#fef3c7",color:"#92400e"},"Not Found":{bg:"#fee2e2",color:"#991b1b"},"Not Working":{bg:"#f3f4f6",color:"#374151"}};
const EMPTY={category:"Webcam",brand:"",model:"",serial:"",assigned:"",status:"Available",location:"",remark:""};
const CATEGORIES=["Webcam","Speaker","Headset","Microphone","UPS","Extension Board","Charger","Docking Station","Cable","Other"];
const STATUSES=["Assigned","Available","Maintenance","Not Found","Not Working"];
const LOCATIONS=["ABIL","TEERTH","AMBROSIA",""];

export default function ExtraEquipment() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const res = await getExtraEquipment();
      const items = res.data?.extraEquipment ?? res.data ?? [];
  ...
  const filtered=useMemo(()=>assets.filter(a=>{const q=search.toLowerCase();return(!q||[a.category,a.brand,a.model,a.serial,a.assigned].some(v=>v?.toLowerCase().includes(q)))&&(!fCat||a.category===fCat)&&(!fStatus||a.status===fStatus)&&(!fLoc||a.location===fLoc);}),[assets,search,fCat,fStatus,fLoc]);
  ...
  const save=async()=>{if(!form.category){alert("Category required.");return;}try{setSaving(true);if(editId){await updateExtraEquipment(editId,form);}else{await createExtraEquipment(form);}await fetchData();setShowModal(false);setEditId(null);}catch(err){setError(err.response?.data?.message||"Save failed.");}finally{setSaving(false);}};
    ...
    <div className="lp-page">
      ...
      <div className="lp-page-header" style={{padding:"16px 20px 0"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10,marginBottom:12}}>
          <div><h1 style={{margin:0,fontSize:20,fontWeight:700,color:"#111827"}}>Extra Equipment</h1><p style={{margin:"2px 0 0",fontSize:12,color:"#6b7280"}}>{assets.length} total assets</p></div>
      ...
      <div className="lp-card" style={{margin:"0 20px 20px"}}>
        <div style={{padding:"10px 14px",borderBottom:"1px solid #E5E7EB",display:"flex",gap:8,flexWrap:"wrap",alignItems:"center",background:"#fafafa"}}>
          <input className="lp-search" placeholder="Search category, brand, assigned…" value={search} onChange={e=>{setSearch(e.target.value);setPagination(p=>({...p,pageIndex:0}));}} style={{width:220}}/>
          <select className="lp-sel lp-sel-sm" value={fCat} onChange={e=>{setFCat(e.target.value);setPagination(p=>({...p,pageIndex:0}));}}><option value="">All categories</option>{CATEGORIES.map(c=><option key={c} value={c}>{c}</option>)}</select>
          ...
          {(search||fCat||fStatus||fLoc)&&<button className="lp-btn-ghost" onClick={()=>{setSearch("");setFCat("");setFStatus("");setFLoc("");setPagination({pageIndex:0,pageSize:15});}}>Clear</button>}
      ...
      {showModal&&<div className="lp-overlay"><div className="lp-modal"><div className="lp-modal-header"><h2 className="lp-modal-title">{editId?"Edit Asset":"Add New Asset"}</h2><button className="lp-modal-close" onClick={()=>{setShowModal(false);setEditId(null);}}>×</button></div><div className="lp-modal-body"><div className="lp-form-grid"><Field field="category" label="Category *" opts={CATEGORIES}/><Field field="brand" label="Brand"/><Field field="model" label="Model"/><Field field="serial" label="Serial No"/><Field field="status" label="Status" opts={STATUSES}/><Field field="assigned" label="Assigned To"/><Field field="location" label="Location" opts={LOCATIONS}/><Field field="remark" label="Remark"/></div></div><div className="lp-modal-footer"><button className="lp-btn-ghost" onClick={()=>{setShowModal(false);setEditId(null);}}>Cancel</button><button className="lp-btn-primary" onClick={save} disabled={saving}>{saving?"Saving…":editId?"Update":"Save"}</button></div></div></div>}
      {delConfirm&&<div className="lp-overlay"><div className="lp-modal lp-modal-sm"><div className="lp-modal-header"><h3 className="lp-modal-title">Delete Asset</h3><button className="lp-modal-close" onClick={()=>setDelConfirm(null)}>×</button></div><div className="lp-modal-body"><p className="lp-del-msg">Delete <strong>{delConfirm.category} — {delConfirm.brand} {delConfirm.model}</strong>?</p></div><div className="lp-modal-footer"><button className="lp-btn-ghost" onClick={()=>setDelConfirm(null)}>Cancel</button><button className="lp-btn-danger" onClick={async()=>{try{await deleteExtraEquipment(delConfirm._id||delConfirm.id);setDelConfirm(null);await fetchData();}catch(err){setError(err.response?.data?.message||"Delete failed.");setDelConfirm(null);}}}>Delete</button></div></div></div>}
    </div>
  );
}
